...
export default function Contact() {
  return (
    <div className="w-full py-12 bg-[var(--color-bg)] flex flex-col items-center text-[var(--color-primary)] px-6">
      <h2 className="text-3xl md:text-5xl font-bold mb-6 text-[var(--color-accent)]">Get in Touch</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl w-full">
        {cards.map((item, index) => (
          <motion.div
            key={index}
            className="bg-[var(--color-homeBg)] p-6 rounded-lg shadow-lg group flex flex-col items-start gap-2 group hover:bg-[var(--color-accent)] transition-colors"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            whileHover={{ scale: 1.05, y: -5 }}
            transition={{ type: "spring", stiffness: 120, damping: 10, delay: index * 0.1 }}
          >
            <h3 className="font-bold text-lg text-[var(--color-primary)] group-hover:text-white transition-colors">
              {item.title}
            </h3>
            <a
              href={item.href}
              ...
              className="flex items-center gap-2 text-[var(--color-accent)] group-hover:text-white hover:underline transition-colors"
            >
              {item.icon} {item.text}
            </a>
          </motion.div>
